...
// -- battery state
//
// Tello flight dynamics are sophisticated and difficult to model. TelloPlugin
// keeps it simple:
...
public:
  ...
  void set_target_velocities(std::string rc_command) {
    ...
    try {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "can't parse rc command '%s', exception %s",
                   rc_command.c_str(), e.what());
      return;
    }

    set_target_velocities(x * MAX_XY_V, y * MAX_XY_V, z * MAX_Z_V,
                          yaw * MAX_ANG_V);
  }
  ...
  void Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf) {
    ...
    // In theory we can move much of this config into the <ros> tag, perhaps
    // it's finished in Eloquent?
    if (sdf->HasElement("link_name")) {
      link_name = sdf->GetElement("link_name")->Get<std::string>();
    }
    if (sdf->HasElement("center_of_mass")) {
      center_of_mass_ =
          sdf->GetElement("center_of_mass")->Get<ignition::math::Vector3d>();
    }
    ...
    // Fix by adding <parameter name="use_sim_time" type="bool">1</parameter> to
    // the SDF file
    bool use_sim_time;
    node_->get_parameter("use_sim_time", use_sim_time);
    if (!use_sim_time) {
      RCLCPP_ERROR(node_->get_logger(),
                   "use_sim_time is false, could be a bug");
    }
    ...
    // Listen for the Gazebo update event. This event is broadcast every
  ...
  void OnUpdate(const gazebo::common::UpdateInfo &info) {
    ...
    if (flight_state_ != FlightState::landed) {
      ...
      ignition::math::Vector3d linear_velocity =
          base_link_->RelativeLinearVel();
      ignition::math::Vector3d angular_velocity =
          base_link_->RelativeAngularVel();
      ...
      ignition::math::Vector3d force =
          lin_ubar * base_link_->GetInertial()->Mass();
      ignition::math::Vector3d torque =
          ang_ubar * base_link_->GetInertial()->MOI();
      ...
      base_link_->AddRelativeTorque(
          torque); // ODE adds torque at the center of mass
    }
  }
...
#pragma ide diagnostic ignored "UnusedValue"
  ...
  void command_callback(
    ...
    if (request->cmd == "takeoff" && flight_state_ == FlightState::landed) {
      ...
      response->rc = response->OK;
    } else if (is_prefix("rc", request->cmd) &&
               flight_state_ == FlightState::flying) {
      set_target_velocities(request->cmd);
      response->rc = response->OK;
    } else {
      RCLCPP_WARN(node_->get_logger(), "ignoring command '%s'",
                  request->cmd.c_str());
...
#pragma clang diagnostic pop
  ...
  void cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg) {
    if (flight_state_ == FlightState::flying) {
      ...
      set_target_velocities(msg->linear.x * MAX_XY_V, msg->linear.y * MAX_XY_V,
  ...
  void spin_10Hz() {
    ...
    // Simulate a battery
    int battery_percent = static_cast<int>(
        (battery_duration_ - ros_time.seconds()) / battery_duration_ * 100);
    ...
    odom_msg.header.frame_id = std::string("odom");
    ...
    odom_msg.twist.twist.linear.x = base_link_->RelativeLinearVel().X();
    odom_msg.twist.twist.linear.y = base_link_->RelativeLinearVel().Y();
    odom_msg.twist.twist.linear.z = base_link_->RelativeLinearVel().Z();

    odom_msg.twist.twist.angular.x = base_link_->RelativeAngularVel().X();
    odom_msg.twist.twist.angular.y = base_link_->RelativeAngularVel().Y();
    odom_msg.twist.twist.angular.z = base_link_->RelativeAngularVel().Z();
